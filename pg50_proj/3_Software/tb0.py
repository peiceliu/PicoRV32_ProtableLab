...
class protocol_analyzer:
 ...
 # ----------- 1-Wire 协议解析 -----------
    ...
    # ----------- 总协议解析入口 -----------
    def analyze(self, protocol, *args, **kwargs):
        if protocol == "1-Wire":
            return self.parse_1wire(*args)
        elif protocol == "I2C":
            return self.parse_i2c(*args)
        elif protocol == "SPI":
            return self.parse_spi(*args, **kwargs)
        elif protocol == "UART":
            return self.parse_uart(*args)
        else:
...
class DataAcquisitionThread(QThread):
    ...
    def receive_udp_packet(self):
        ...
        self.UDP_PORT=32896
        self.packet_count = int(self.sample_depth // 20)
...
class LogicAnalyzerApp(QWidget):  # 合并后的类
    def __init__(self):
        super().__init__()
    ...
    def update_waveform(self, data, time):
        ...
        self.current_index = 0
        # 一次画图的步进值为100
        self.step_size = 100 # len(self.time) // 100
    ...
    def plot_next_segment(self):
        ...
        if self.current_index > (len(self.time) // 100):
            self.timer.stop()
            self.log_protocol.append("波形更新完成")
            return
        start_index = self.current_index * self.step_size
        end_index = (self.current_index + 1) * self.step_size if self.current_index < len(self.time)//100 else len(self.time)
